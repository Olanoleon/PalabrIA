...
function prompt(input: GenerationInput): string {
  ...
  lines.push(
    ...
    `- TYPE_WHAT_YOU_HEAR: the learner spells the word from audio. "options" must be an empty array, answerIndex 0, "sentence" null, "pairs" null. Entries of one or two words are both fine — the keypad shows a gap between the words and the learner types letters only — but never choose an entry of three words or more.`,
    `- MATCH_UP: the learner pairs ${MATCH_PAIRS} English words with their Spanish meanings. Fill "pairs" with exactly ${MATCH_PAIRS} objects of {en, es}, where "en" is one of this unit's words and "es" is its Spanish meaning. Set "word" to the first pair's English word, "options" to an empty array, answerIndex 0, "sentence" null. The ${MATCH_PAIRS} meanings must be clearly different from one another, or the exercise is guesswork.`,
