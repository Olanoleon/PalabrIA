...
function prompt(input: GenerationInput): string {
  ...
  lines.push(
    ...
    `- TYPE_WHAT_YOU_HEAR: the learner spells the word from audio. "options" must be an empty array, answerIndex 0, "sentence" null, "pairs" null. Entries of one or two parts are both fine — a hyphen counts as a separator like a space, the keypad shows a gap there and the learner types letters only — but never choose an entry of three parts or more ("father-in-law", "maid of honor"). Prefer entries under 14 letters: the slots wrap on a phone beyond that, so pick "grandfather" over "great-grandfather" when both are available.`,
