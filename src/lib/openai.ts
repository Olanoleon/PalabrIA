...
function prompt(input: GenerationInput): string {
  ...
  lines.push(
    ...
    `- A FILL_BLANK sentence must leave exactly ONE option possible. Write the sentence first, carrying the clue that rules the other three out, then test every distractor against it: if a distractor could also finish the sentence truthfully, the question has no answer and the learner is guessing. «My ______ has two children.» is wrong — a stepmother, a mother-in-law, a sister-in-law and a daughter-in-law can each have two children. «My ______ is my husband's mother.» is right: nothing else fits. The clue has to be what the word uniquely IS, does or names — a definition folded into a sentence — never a detail every candidate could share (having children, helping at home, living nearby, being kind).`,
    `- Choose distractors from this unit's own words. If three of them would need the very same clue to rule out, the sentence is not specific enough yet — rewrite it rather than swapping the options.`,
    `- IPA_MATCH runs one way only: the question names the WORD and the options are transcriptions. "prompt"/"promptEs" must contain the word itself and ask which transcription is its pronunciation — «Which transcription matches "backache"?» / «¿Qué transcripción corresponde a "backache"?» — and "options" holds 4 IPA transcriptions between slashes, one of them exactly this word's own "ipa". Set answerIndex to it, "pairs" null.`,
    `- NEVER put IPA in an IPA_MATCH question. «Which word has this pronunciation: /ˈbæk.eɪk/?» with transcriptions as the options asks the learner to match a transcription against itself, and the word never appears on the screen at all. The prompt carries the spelling; the options carry the sound.`,
